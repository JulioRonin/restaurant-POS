...
export default function SuperAdminScreen() {
  ...
  const toggleBusinessStatus = async () => {
    ...
    try {
      ...
      alert(`Negocio ${newStatus ? 'activado' : 'bloqueado'} con éxito.`);
    } catch (err) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="mt-10 pt-10 border-t border-slate-800 space-y-6">
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2">
                        ...
                        Cuentas Vinculadas ({selectedBusiness.profiles?.length || 0})
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {selectedBusiness.profiles?.map((profile: any, idx: number) => (
                            <div key={idx} className="bg-slate-900/40 border border-slate-800 p-4 rounded-2xl flex items-center justify-between group">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-xs font-bold text-[#1a1c14]">{profile.full_name}</p>
                                        <p className="text-[9px] text-slate-600 font-medium uppercase tracking-wider">{profile.role}</p>
                                    </div>
                                </div>
                                <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                                    <span className="text-[8px] font-black text-slate-700 uppercase">Managed by Node</span>
                                </div>
                            </div>
                        ))}
                    </div>
               ...
               <div className="mt-12 pt-8 border-t border-slate-800 flex items-center justify-between">
                  ...
                  <div className="flex gap-4">
                    <button 
                        ...
                        disabled={saving}
                        className={`flex items-center gap-2 font-bold text-xs px-4 py-2 rounded-xl transition-all disabled:opacity-50 ${
                            selectedBusiness.is_active 
                            ? 'bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 border border-amber-500/20' 
                            : 'bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 border border-emerald-500/20'
                        }`}
                    >
                        {selectedBusiness.is_active ? <XCircle className="w-4 h-4" /> : <CheckCircle2 className="w-4 h-4" />}
                        {selectedBusiness.is_active ? 'Bloquear Cuenta' : 'Desbloquear Cuenta'}
                    </button>
                    <button 
                        onClick={handleDeleteBusiness}
                        disabled={saving}
                        className="flex items-center gap-2 bg-red-500/10 text-red-500 hover:bg-red-500/20 border border-red-500/20 font-bold text-xs px-4 py-2 rounded-xl transition-all disabled:opacity-50"
                    >
